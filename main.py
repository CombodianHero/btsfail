"""
Bridge to Success — No-Login Batch Extractor (Web Service)
============================================================
Same extraction logic as the original CLI script
`bridgetosuccess_nologin_extractor.py`, wrapped in a Flask app so it
can be deployed on Koyeb (or any container host).
...
What this service exposes (no login needed):
  ...
  - Full text-dump extraction (single course or all courses), downloadable

What requires login (NOT exposed here):
  - Enrolled course videos (myCourseVideo) for paid courses
  - Enrolled course PDFs  (myCoursePdf) for paid courses
  - Live class links
...
Deploy on Koyeb:
    ...
    - Create a new Koyeb Web Service from the repo
    - Koyeb auto-detects the Dockerfile (or use the included Procfile
      with the Python buildpack)
    - Koyeb sets the PORT env var automatically; main.py reads it.
------------------------------------------------------------------
"""
...
@handle_errors
def api_courses_all():
    return jsonp(count=len(fetch_all_courses()), courses=fetch_all_courses())


@app.route("/api/courses/top")
@handle_errors
def api_courses_top():
    courses = fetch_top_courses()
...
def api_course_info(course_id):
    info = fetch_course_info(course_id)
    if not info:
...
def api_free_content():
    videos = fetch_free_videos()
    pdfs = fetch_free_pdfs()
    return jsonp(
...
def api_configuration():
    return jsonp(configuration=fetch_configuration())
...
def api_course_report(course_id):
    combined = get_combined_courses()
    course = next(
        (c for c in combined if get_field(c, "id", "courseId", "course_id") == str(course_id)),
        None,
    )
    if not course:
        return jsonp(success=False, error="Course not found"), 404

    report, stats = build_full_report([course])

    if request.args.get("download"):
        title_slug = sanitize(get_field(course, "title", "courseName", "name") or "course")
...
def api_report_all():
    ...
    report, stats = build_full_report(combined)
...
def api_report_list():
    combined = get_combined_courses()
    report = build_course_list_report(combined)
